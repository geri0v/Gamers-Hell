...
const CACHE_KEY = 'gw2_event_data_v1'; // unique per app/version
...
export function saveEventCache(events) {
  try {
    ...
    // If quota exceeded, warn user or ignore silently
    return false;
  }
}
...
export function loadEventCache() {
  try {
    ...
    const expired = Date.now() - data.timestamp > MAX_AGE_MS;
    return expired ? null : data.events;
  } catch {
    return null;
  }
}
