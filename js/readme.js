...
export async function showReadmeModal() {
  ...
  try {
    ...
    else md = "# README could not be loaded.";
  } catch {
    md = "# README could not be loaded.";
  }
  const m = document.createElement('div');
  m.id = "modal-readme";
  Object.assign(m.style, {
    position:'fixed',top:0,left:0,right:0,bottom:0,background:'rgba(24,32,54,.75)',
    display:'flex',alignItems:'center',justifyContent:'center',zIndex: 10001
  });
  let html = `<div style="background:#fff;border-radius:10px;padding:2em;max-width:500px;max-height:85vh;overflow-y:auto;text-align:left;">
