...
(function() {
  ...
  // --- 3. App Logic ---
  // Configurable JSON sources: add more as needed!
  const jsonSources = [
    {
      name: "Temples of Orr",
  ...
  async function renderAllSections(events, searchQuery, sortBy) {
    const allSectionsDiv = document.getElementById('allSections');
  ...
  // --- 4. Search, Clear, Sort Handlers ---
  document.getElementById('searchInput').addEventListener('input', function() {
    currentSearch = this.value.trim();
    document.getElementById('searchClearBtn').style.display = currentSearch ? '' : 'none';
  ...
  // --- 5. Modal Logic (Optional, for future expansion) ---
  ...
  // --- 7. Add JSON sources easily! ---
  // To add more sources, just push to jsonSources:
  // jsonSources.push({ name: "Your Source", url: "https://..." });
