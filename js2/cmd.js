// == Gamers-Hell: Full JS App with Global CSS ==
...
(function() {
  ...
  const jsonSources = [
    {
      name: "Temples of Orr",
      url: "https://geri0v.github.io/Gamers-Hell/json/core/temples.json"
    },
    {
      name: "Valuable Events",
      url: "https://geri0v.github.io/Gamers-Hell/json/core/untimedcore.json"
    }
  ];
  ...
  async function fetchAllEvents() {
    const all = await Promise.all(jsonSources.map(async (src) => {
      try {
        ...
        let events = [];
        let sourceName = src.name || 'Unknown Source';
  ...
  function sortEvents(events, sortBy, tpPrices) {
    if (sortBy === "location") {
      return [...events].sort((a, b) => (a.map || a.location || '').localeCompare(b.map || b.location || ''));
    }
    if (sortBy === "loot") {
      return [...events].sort((a, b) => {
        const aMax = Math.max(...((a.loot || []).map(item => (item.id && tpPrices[item.id] && tpPrices[item.id].sell) || 0)), 0);
        const bMax = Math.max(...((b.loot || []).map(item => (item.id && tpPrices[item.id] && tpPrices[item.id].sell) || 0)), 0);
        return bMax - aMax;
      });
    }
    // Default: name
  ...
  function renderEventCard(event, idx, searchQuery, tpPrices, tpIcons, maxSellValue) {
    ...
    if (Array.isArray(event.loot) && event.loot.length) {
      lootRows = event.loot.map(item => {
        ...
        let wikiLink = item.id ? `<a href="https://wiki.guildwars2.com/wiki/Special:Search/${encodeURIComponent(item.name || '')}" target="_blank" rel="noopener" aria-label="GW2 Wiki for ${item.name}" style="margin-left:0.3em;color:#4a90e2;text-decoration:underline;">Wiki</a>` : '';
        let effLink = item.id ? `<a href="https://gw2efficiency.com/item/${item.id}" target="_blank" rel="noopener" aria-label="GW2Efficiency for ${item.name}" style="margin-left:0.3em;color:#4a90e2;text-decoration:underline;">Efficiency</a>` : '';
        return `<tr>
  ...
  window.renderAllSections = async function(events, searchQuery, sortBy) {
    ...
    if (!filtered.length) {
      eventsContainer().innerHTML = `<div class="empty-state"><img src="https://i.imgur.com/8z8Q2Hk.png" alt="No events"><div>No events found. Try a different search or reload data.</div></div>`;
      return;
    }
    ...
    for (const [expansion, sources] of Object.entries(expansions)) {
      ...
      for (const [sourceName, evs] of Object.entries(sources)) {
        const sortedEvs = sortEvents(evs, sortBy, tpPrices);
        html += `
  ...
  // --- 9. Search, Clear, Sort Handlers ---
  document.addEventListener('DOMContentLoaded', function() {
    searchInput().addEventListener('input', function() {
      window.currentSearch = this.value.trim();
      searchClearBtn().style.display = window.currentSearch ? '' : 'none';
      window.renderAllSections(window.allEvents, window.currentSearch, window.currentSort);
    });
